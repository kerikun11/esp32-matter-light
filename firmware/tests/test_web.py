...
sys.path.insert(0, str(TOOLS.parent / "components/device_common/tools/web"))
from build_web import build, generate, render_source
...
class WebTests(unittest.TestCase):
    ...
    def test_compression_is_deterministic_and_round_trips(self):
        ...
        self.assertLess(len(plain), len(render_source(server.TEMPLATE).encode()))
        self.assertLess(len(compressed), len(plain) // 2)
        self.assertNotIn(b'{{', plain)
        self.assertIn('読込中'.encode(), plain)

    def test_settings_layout_and_reboot(self):
        source = server.TEMPLATE.read_text(encoding="utf-8")
        self.assertLess(source.index("赤外線リモコン学習"), source.index("デバイス情報"))
        self.assertIn("/reboot", source)
        status, _, body = self.request(
            "/reboot", {"Accept": "application/json"}, {})
        self.assertEqual(status, 200)
        self.assertIn("再起動", json.loads(body)["message"])

    def test_shared_settings_layout_and_pairing(self):
        source = server.TEMPLATE.read_text(encoding="utf-8")
        self.assertNotIn('class="two-col"', source)
        self.assertIn('id="settings-form"', source)
        self.assertIn('id="pairing" class="pairing" hidden', source)
        self.assertIn('id="manual-code"', source)
        self.assertIn('id="qr-link"', source)
        common_css = (TOOLS.parent / "components/device_common/web/styles.css").read_text()
        self.assertIn("#preview{margin:0 0 14px}", common_css)
        info = json.loads(self.request('/device-info')[2])
        self.assertRegex(info['manual_code'], r'^\d{11}$')
        self.assertTrue(info['qr_payload'].startswith('MT:'))
...
''')
            subprocess.run(['g++', '-std=c++17', '-Wall', '-Wextra', '-Werror',
                            '-I' + str(TOOLS.parent / 'components/device_common/include'), '-I' + str(tmp),
